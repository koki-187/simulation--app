...
export function ExportBar({ resultA, resultB = null }: Props) {
  ...
  return (
    <div className="flex items-center gap-2" role="group" aria-label="エクスポート">
      ...
      >
        {pdfLoading ? (
          <svg className="w-3.5 h-3.5 animate-spin text-orange-500" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
          </svg>
        ) : (
          <svg className="w-3.5 h-3.5 text-danger-500" viewBox="0 0 16 16" fill="currentColor">
      ...
      >
        {xlsLoading ? (
          <svg className="w-3.5 h-3.5 animate-spin text-success-500" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
          </svg>
        ) : (
          <svg className="w-3.5 h-3.5 text-success-500" viewBox="0 0 16 16" fill="currentColor">
